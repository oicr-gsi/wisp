...
workflow wisp {
  ...
  call purple as purplePlasma {
    input:
      normal_name = extractNormalName.input_name,
      tumour_name = extractPlasmaName.input_name,
      amber_directory = amberPlasma.output_directory,
      cobalt_directory = cobaltPlasma.output_directory,
      somatic_vcf = mergePlasmaVcfs.merged_vcf,
      genomeVersion = genomeVersion,
      modules = resources[genomeVersion].hmfModules,
      gcProfile = resources[genomeVersion].gcProfile,
      ensemblDir = resources[genomeVersion].ensemblDir,
  ...
  call annotatePrimaryVcfWithPurple {
    input:
      purple_vcf = purplePrimary.purple_somatic_vcf,
      purple_vcf_index = purplePrimary.purple_somatic_vcf_index,
  ...
  call runWisp {
    input:
      ...
      purple_dir = purplePrimary.purple_directory,
      amber_dir = mergeAmber.output_zip,
      cobalt_dir = mergeCobalt.output_zip,
      somatic_vcf = purplePlasma.purple_somatic_vcf,
      somatic_vcf_index = purplePlasma.purple_somatic_vcf_index,
